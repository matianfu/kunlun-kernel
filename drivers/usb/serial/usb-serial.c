...
extern void ap_bind_usb_device(struct usb_device *dev);
extern void ap_unbind_usb_device(void);
...
/* Temp workaround, avoid crash when kill rild, need check */
static struct tty_struct * serial_lookup(struct tty_driver *driver, struct inode *inode, int idx)
{
    dbg("%s: enter\n");
    return NULL;
}
