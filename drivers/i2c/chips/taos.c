...
struct taos_data {
    ...
    struct cdev cdev;
    unsigned int addr;
    struct input_dev *input_dev_als;//iVIZM
    struct input_dev *input_dev_prox;//iVIZM
...
static u8 prox_intr_filter_param = 0x21;//0x23;//zjj 0x33  //wj 0x23
static u8 prox_config_param = 0x00;
static u8 prox_pulse_cnt_param = 0x08;
static u8 prox_gain_param = 0x62;//zjj 0x22
...
static int taos_als_get_data(void)//iVIZM
{
	...
	{
	   printk(KERN_ERR "111111111  TAOS: taos_als_get_data  reg_val === %0x\n", reg_val);
		return -ENODATA;
	}
   ...
   if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG | TAOS_TRITON_STATUS)))) < 0) {
	...
	{
		{
	   printk(KERN_ERR "222222222222222  TAOS: taos_als_get_data  reg_val === %0x\n", reg_val);
		return -ENODATA;
	}
	}
    ...
    TAOSDEBUG("********** before taos_als_get_data ********* lux_val = %d \n",lux_val);
   ...
   input_sync(taos_datap->input_dev_als);
    ...
    TAOSDEBUG("********** taos_als_get_data ********* lux_val = %d \n",lux_val);
    return ret;
}
...
static int taos_prox_threshold_set(void)//iVIZM
{
    ...
    if (prox_on || proxdata < taos_cfgp->prox_threshold_lo ) {
	  ...
	  input_report_abs(taos_datap->input_dev_prox,ABS_DISTANCE,data);
...
static void als_calibrate(struct taos_cfg *taos_cfg, struct taos_data *taos_data)
{  int itime = 0;
   int ret = 0;
...
static void prox_calibrate(struct taos_cfg *taos_cfg, struct taos_data *taos_data)
{
   int itime = 0, i = 0;
   int ret = 0;
   u8  reg_cntrl = 0,reg_val = 0;
...
static int taos_probe(struct i2c_client *clientp, const struct i2c_device_id *idp) {
   ...
   TAOSDEBUG(KERN_ERR "TAOS: taos_probe ********111111111111****** in taos_probe()\n");

   taos_datap->input_dev_als = input_allocate_device();//iVIZM
   taos_datap->input_dev_prox = input_allocate_device();//iVIZM
   if ((taos_datap->input_dev_als == NULL)||(taos_datap->input_dev_prox == NULL)) {
       return -ENOMEM;
   }

   TAOSDEBUG(KERN_ERR "TAOS: taos_probe ********222222222222****** in taos_probe()\n");
   ...
   input_set_capability(taos_datap->input_dev_prox,EV_ABS,ABS_DISTANCE);
   input_set_capability(taos_datap->input_dev_als,EV_ABS,ABS_MISC);
   ...
   ret = input_register_device(taos_datap->input_dev_prox);
	...
	TAOSDEBUG(KERN_ERR "TAOS: taos_probe ********333333333333333****** in taos_probe()\n");
    ...
    TAOSDEBUG("Rambo::light sensor will pwr down \n");
    ...
    ret = gpio_request(TAOS_INT_GPIO,"ALS_PS_INT");//wj
    if (ret < 0) {
        printk("failed to request GPIO:%d,ERRNO:%d\n",(int)TAOS_INT_GPIO,ret);
       return(ret);
    }
    gpio_direction_input(TAOS_INT_GPIO);//wj


   TAOSDEBUG(KERN_ERR "TAOS: taos_probe ********444444444444444444****** in taos_probe()\n");
    //set_irq_type(gpio_to_irq(113), IRQ_TYPE_EDGE_FALLING);
    ...
    als_calibrate(taos_cfgp, taos_datap);
    prox_calibrate(taos_cfgp, taos_datap);
    return (ret);
}
...
static int taos_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    case TAOS_IOCTL_SENSOR_CHECK:
        ...
        TAOSDEBUG(KERN_ERR "TAOS: TAOS_IOCTL_SENSOR_CHECK,prox_adc_time,%d~\n",reg_val_temp);
     ...
     case TAOS_IOCTL_SENSOR_ON:
	   ...
	   TAOSDEBUG(KERN_ERR" TAOS IOCTL ALS ON   reg_val === %0x\n", reg_val);
        ...
        case TAOS_IOCTL_CONFIG_SET:
            TAOSDEBUG("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n");
            TAOSDEBUG("^^^^^^^^^ TAOS INCTL CONFIG SET  ^^^^^^^\n");
            TAOSDEBUG("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n");
            ret = copy_from_user(taos_cfgp, (struct taos_cfg *)arg, sizeof(struct taos_cfg));
            if (ret) {
                printk(KERN_ERR "TAOS: copy_from_user failed in ioctl config_set\n");
...
static int taos_get_lux(void) {
	...
	raw_ir    <<= 8;
	raw_ir    |= chdata[2];
	printk("wj------taos_get_lux--raw_clear==%d\n",raw_clear);
	printk("wj------taos_get_lux--raw_ir==%d\n",raw_ir);
	...
	printk("wj------taos_get_lux--ratio==%d\n",ratio);
...
static int taos_get_lux(void) {
    ...
    raw_clear *= (taos_cfgp->scale_factor *7);
    raw_ir *= (taos_cfgp->scale_factor*3 );
