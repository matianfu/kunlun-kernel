...
static int option_incepted_write(struct tty_struct *tty, struct usb_serial_port
...
#ifdef CONFIG_USB_ANDROID_RAWBULK
static int option_incepted_write(struct tty_struct *tty, struct usb_serial_port *port,
...
static int option_rawbulk_intercept(struct usb_interface *interface, int enable) {
    int rc = 0;
    int n;
...
static int usb_serial_option_probe(struct usb_interface *interface,
    ...
    if (ret < 0)
        return ret;
#ifdef CONFIG_USB_ANDROID_RAWBULK
    return rawbulk_bind_host_interface(interface, option_rawbulk_intercept);
#else
    return 0;
#endif
}
