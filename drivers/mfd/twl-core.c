...
#define DRIVER_NAME			"twl"
	...
	defined(CONFIG_TWL4030_BCI_CHARGE) || \
	defined(CONFIG_TWL4030_BCI_CHARGE_MODULE) || \
...
add_children(struct twl4030_platform_data *pdata, unsigned long features)
{
	...
	if (twl_has_regulator() && twl_class_is_6030()) {
		...
		child = add_regulator(TWL4030_REG_KBL, pdata->kbl);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(SWITCH_GPIO_GPS, pdata->gps);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(SWITCH_GPIO_CAM, pdata->cam);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(SWITCH_GPIO_WLAN, pdata->wlan);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(SWITCH_GPIO_BT, pdata->bt);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(SWITCH_GPIO_FM, pdata->fm);
		if (IS_ERR(child))
			return PTR_ERR(child);
#endif
	}
