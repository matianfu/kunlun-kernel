...
static struct snd_soc_codec *twl4030_socdev;
...
extern enable_headset_button_irq(int enable);
/*
...
static void twl4030_init_chip(struct snd_soc_codec *codec)
{
	...
	value = twl4030_read_reg_cache(codec, TWL4030_REG_MICBIAS_CTL);
	value |= TWL4030_HSMICBIAS_EN;
	twl4030_write(codec, TWL4030_REG_MICBIAS_CTL, value);
...
{
	switch (event) {
        /* add this to resolve left channel has no sound when playing stereo music(the left channel of this mp3 has no sound)  */
	case SND_SOC_DAPM_POST_PMU:
                twl4030_write(w->codec, TWL4030_REG_RX_PATH_SEL, 0x20);
		handsfree_ramp(w->codec, TWL4030_REG_HFL_CTL, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
                twl4030_write(w->codec, TWL4030_REG_RX_PATH_SEL, 0x00);
...
static const struct snd_kcontrol_new twl4030_snd_controls[] = {
	...
	//	TWL4030_REG_VRXPGA, 0, 0x31, 0, digital_voice_downlink_tlv),
  	...
  	SOC_SINGLE_TLV_ENHANCE("DAC Voice Digital Downlink Volume",
		TWL4030_REG_VRXPGA, 0, 0x28, 0x1d, 0, digital_voice_downlink_tlv), //Modified by Hyan to support control min volume while calling
    ...
    SOC_SINGLE("HYAN Voice sidetone gain",
	       TWL4030_REG_VSTPGA, 0, 0x29, 0),	

    SOC_SINGLE("HYAN ADC Physical Left",
	       TWL4030_REG_AVADC_CTL, 3, 1, 0),

    SOC_SINGLE("HYAN MIC BIAS1_EN",
	       TWL4030_REG_MICBIAS_CTL, 0, 1, 0),

    SOC_SINGLE("HYAN HEADSET MIC BIAS1_EN",
	       TWL4030_REG_MICBIAS_CTL, 2, 1, 0),

//	SOC_SINGLE("ARXR2 FM LOOPBACK EN",
//           TWL4030_REG_ARXR2_APGA_CTL, 2, 1, 0),

//	SOC_SINGLE("ARXL2 FM LOOPBACK EN",
//           TWL4030_REG_ARXL2_APGA_CTL, 2, 1, 0),

    SOC_SINGLE("ARXR2 DAC APGA EN",
           TWL4030_REG_ARXR2_APGA_CTL, 0, 1, 0),

    SOC_SINGLE("ARXL2 DAC APGA EN",
           TWL4030_REG_ARXL2_APGA_CTL, 0, 1, 0),

    SOC_SINGLE("ARXR1 DAC APGA EN",
           TWL4030_REG_ARXR1_APGA_CTL, 0, 1, 0),

    SOC_SINGLE("ARXL1 DAC APGA EN",
           TWL4030_REG_ARXL1_APGA_CTL, 0, 1, 0),

	SOC_SINGLE("FM LOOPBACK EN",
           TWL4030_REG_MISC_SET_1, 5, 1, 0),

	       
    SOC_SINGLE("HYAN Mute HandsfreeR",
	       TWL4030_REG_HFR_CTL, 0, 0, 0),

    SOC_SINGLE("HYAN Mute HandsfreeL",
	       TWL4030_REG_HFL_CTL, 0, 0, 0),

    SOC_SINGLE("HYAN Mute EARPIECE",
	       TWL4030_REG_EAR_CTL, 0, 0, 0),

    SOC_SINGLE("HYAN PCM HIGH IMPEDANCE",
	...
	SOC_SINGLE("DTMF Playback Gain Tone1",
		TWL4030_REG_DTMF_PGA_CTL1, 0, 0x0f, 0),


	SOC_SINGLE("DTMF Playback Gain Tone2",
		TWL4030_REG_DTMF_PGA_CTL1, 4, 0x0f, 0),

	SOC_SINGLE("DTMF Playback Gain Tone3",
		TWL4030_REG_DTMF_PGA_CTL2, 0, 0x07, 0),

	SOC_SINGLE("DTMF Playback Gain Tone4",
		TWL4030_REG_DTMF_PGA_CTL2, 4, 0x07, 0),
    ...
    SOC_SINGLE("Analog Capture Volume Left",
        TWL4030_REG_ANAMIC_GAIN, 0, 5, 0),

    SOC_SINGLE("Analog Capture Volume Right",
        TWL4030_REG_ANAMIC_GAIN, 3, 5, 0),
	...
	SOC_SINGLE("Analog FM RX Left Input Switch",
				TWL4030_REG_ANAMICL, 2, 1, 0),
	...
	SOC_SINGLE("Analog FM RX Right Input Switch",
				TWL4030_REG_ANAMICR, 2, 1, 0),
...
static const struct snd_soc_dapm_widget twl4030_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC("DAC Voice", "Voice HiFi Playback",
			SND_SOC_NOPM, 0, 0),
	...
	SND_SOC_DAPM_MICBIAS("Headset Mic Bias", SND_SOC_NOPM, 0, 0),
...
int codec_writeAreg(unsigned int reg,unsigned char value)
{
    struct snd_soc_codec *codec = twl4030_socdev;
    struct twl4030_priv *twl4030 = snd_soc_codec_get_drvdata(codec);
    int ret = 0;
    /* Fix loopback problem, force earpiece_enabled true 
      * if audio test requires so that values can be set by twl4030_write*/
    if(reg == TWL4030_REG_EAR_CTL) {
        if(value & 0xf) {
            twl4030->earpiece_enabled = 1;
        } else {
            twl4030->earpiece_enabled = 0;
        }
    }
    ret = twl4030_write(codec,reg,value);
    return ret;
}

int codec_readAreg(unsigned int reg,unsigned char *value)
{
    struct snd_soc_codec *codec = twl4030_socdev;

    *value = twl4030_read_reg_cache(codec, reg);

	return 0 ;
}

EXPORT_SYMBOL(codec_writeAreg);
EXPORT_SYMBOL(codec_readAreg);
...
static int twl4030_soc_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	gpio_set_value(OMAP_PCM_EN_GPIO,0);
	return 0;
}
...
static int twl4030_soc_resume(struct snd_soc_codec *codec)
{
	...
	gpio_set_value(OMAP_PCM_EN_GPIO,1);
	return 0;
}

static int twl4030_is_incall(struct snd_soc_codec *codec)
{
    u8 mode = 0;
...
static int twl4030_soc_probe(struct snd_soc_codec *codec)
{
	...
	codec->is_incall = twl4030_is_incall;
    ...
    if (gpio_request(OMAP_PCM_EN_GPIO, "pcm_en") == 0) {
		gpio_direction_output(OMAP_PCM_EN_GPIO, 1);
		gpio_set_value(OMAP_PCM_EN_GPIO,1);
	}
	twl4030_socdev = codec;
	return 0;
}
