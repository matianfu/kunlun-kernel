...
/*indicate whether suspended during a call*/
static int suspended_incall = 0;
...
static int soc_suspend(struct device *dev)
{
	...
	for (i = 0; i < card->num_rtd; i++) {
		...
		if(codec && codec->is_incall) {
			if(codec->is_incall(codec))
			{
				/* suspend all pcms */
				printk("soc_suspend in call \n");
			    for (i = 0; i < card->num_links; i++){
...
/**
 * Added by Hyan, to support control  the min value of volume reg
 * snd_soc_info_volsw_enhance - enhanced single mixer info callback
 * @kcontrol: mixer control
 * @uinfo: control element information
 *
 * Callback to provide information about a single mixer control.
...
EXPORT_SYMBOL_GPL(snd_soc_info_volsw_enhance);
