...
/* seconds advanced for poweroff alarm */
#define POWEROFF_ALARM_ADJUST_SECONDS 120
#define poweroff_alarm_adjust_time(time) (time - POWEROFF_ALARM_ADJUST_SECONDS)
...
/*
 * Routines for PWOFF RTC wake up.
 */

int alarm_set_rtc_wkalrm(struct timespec *t)
{
	...
	wkalrm.enabled = 1;
	wkalrm.pending = 0;
	rtc_time_to_tm(poweroff_alarm_adjust_time(t->tv_sec), &wkalrm.time);

	mutex_lock(&alarm_setrtc_mutex);
	wake_lock(&alarm_rtc_wake_lock);

	r = rtc_set_alarm(alarm_rtc_dev, &wkalrm);
...
void alarm_clear_rtc_wkalrm(void)
{
	struct rtc_wkalrm wkalrm;

	memset(&wkalrm, 0, sizeof(wkalrm));
	wkalrm.enabled = 0;
	wkalrm.pending = 0;
	...
	wkalrm.time.tm_year = 70;

	mutex_lock(&alarm_setrtc_mutex);
	wake_lock(&alarm_rtc_wake_lock);

	rtc_set_alarm(alarm_rtc_dev, &wkalrm);
