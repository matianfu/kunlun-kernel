...
/**
 * isp_set_cntclk - Configures the specified CNTCLK to the desired frequency.
 * @dev: Device pointer specific to the OMAP3 ISP.
 * @xclk: Desired frequency of the clock in Hz.
 *
 * Configures the specified CNTCLK divisor in the ISP timing control register
 * (TCTRL_CTRL) to generate the desired xclk clock value.
 *
 * Divisor = mclk / cntclk
 *
 * Returns the final frequency that is actually being generated
 **/
u32 isp_set_cntclk(struct device *dev, u32 cntclk)
{
	u32 divisor;
	u32 currentcntclk;
	struct isp_device *isp = dev_get_drvdata(dev);
...
//printk("--------------------func:%s isp->mclk:%ld\n", __func__, isp->mclk);
	if (cntclk >= isp->mclk) {
		...
		divisor = isp->mclk / cntclk;
		if (divisor >= 0x1FF)
			divisor = 0x1FF - 1;
		currentcntclk = isp->mclk / divisor;
	} else {
		divisor = 0;
		currentcntclk = 0;
	}
      ...
      isp_reg_and_or(dev, OMAP3_ISP_IOMEM_MAIN, ISP_TCTRL_CTRL,
			       ~ISPTCTRL_CTRL_DIVC_MASK,
			       divisor << ISPTCTRL_CTRL_DIVC_SHIFT);
