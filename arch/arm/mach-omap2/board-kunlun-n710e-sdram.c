/*
 * SDRC register values for Kunlun
 *
...
static struct omap_sdrc_params kunlun_sdrc_params[4];

/* According to the datasheet of TYBC0A111119L840 from Haier. */
static const struct sdram_timings kunlun_timings[] = {
	{
		.casl = 3,
		.tDAL = 33000,
		.tDPL = 15000,
...
#define SDRC_SET_ONE(reg, st, end, field, rate) \
	if (set_sdrc_timing_regval((reg), (st), (end), \
			kunlun_timings->field, (rate), #field) < 0) \
		err = -1;
#else
#define SDRC_SET_ONE(reg, st, end, field, rate) \
	if (set_sdrc_timing_regval((reg), (st), (end), \
			kunlun_timings->field) < 0) \
		err = -1;
#endif
...
static int sdrc_timings(int id, long rate)
{
	...
	rfr = kunlun_timings[0].tREF * ticks_per_ms / 1000000;
	if (rfr > 65535 + 50)
		rfr = 65535;
	else
		rfr -= 50;
	...
	kunlun_sdrc_params[id].rate = rate;
	kunlun_sdrc_params[id].actim_ctrla = actim_ctrla;
	kunlun_sdrc_params[id].actim_ctrlb = actim_ctrlb;
	kunlun_sdrc_params[id].rfr_ctrl = rfr_ctrl;
	kunlun_sdrc_params[id].mr = 0x32;

	kunlun_sdrc_params[id + 1].rate = 0;

	printk("SDRC param[%d]: rate: %d, actim_ctrla: 0x%lx, actim_ctrlb: 0x%lx, rfr_ctrl: 0x%lx.\n", id, rate, actim_ctrla, actim_ctrlb, rfr_ctrl);
...
struct omap_sdrc_params *kunlun_get_sdram_timings(void)
{
	...
	err = sdrc_timings(0, 166000000);
	err = sdrc_timings(1, 100000000);

	return &kunlun_sdrc_params[0];
}
