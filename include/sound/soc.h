...
#define SOC_SINGLE_VALUE_ENHANCE(xreg, xshift, xmax, xmin, xinvert) \
	((unsigned long)&(struct soc_mixer_control) \
	{.reg = xreg, .shift = xshift, .rshift = xshift, .max = xmax, .min = xmin, \
	.invert = xinvert})
...
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
	...
	.put = snd_soc_put_volsw, \
	.private_value =  SOC_SINGLE_VALUE(reg, shift, max, invert) }
#define SOC_SINGLE_TLV_ENHANCE(xname, reg, shift, max, min, invert, tlv_array) \
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
	.access = SNDRV_CTL_ELEM_ACCESS_TLV_READ |\
		 SNDRV_CTL_ELEM_ACCESS_READWRITE,\
	.tlv.p = (tlv_array), \
	.info = snd_soc_info_volsw_enhance, .get = snd_soc_get_volsw,\
	.put = snd_soc_put_volsw, \
	.private_value =  SOC_SINGLE_VALUE_ENHANCE(reg, shift, max, min, invert) }
...
int snd_soc_info_volsw(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo);
int snd_soc_info_volsw_enhance(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo);
...
struct snd_soc_codec {
	...
	int (* is_incall)(struct snd_soc_codec *);
